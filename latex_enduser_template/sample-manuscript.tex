...
\noindent\textbf{Study Design: Through a within subjects study with 10 participants from the chemistry domain, we evaluated the feature's impact on performance, usability, and perceived workload.}

\noindent\textbf{Results: Automated guidance increased reuse adoption from 0\% in the standard OpenRoberta version to 100\% when using the Reuse Assistant. The feature achieved high usability scores (SUS mean: 84.1) and imposed minimal cognitive burden (NASA-TLX mean: 1.88). The significant order effect revealed that prior manual experience helps users appreciate automation benefits.}
...
%The results showed that automated guidance increased reuse adoption from 0\% to 100\%, achieved high usability scores (SUS mean: 84.1), and imposed minimal cognitive burden (NASA-TLX mean: 1.88).
% You dont present any kind of results in the introduction!! You can very briefly summarize results in the abstract, bc its an overview of the whole report, but the intro is just an introduction to the rest of the report!!

The contributions of this work are both theoretical and practical. We extend the Attention Investment Model \cite{BlackwellAttentionInvestment} by demonstrating that 
proactive assistance can transform feature adoption from a high-cost investment to a low-cost opportunistic choice. 
We also extend Ko et al.'s Learning Barriers Framework \cite{KoLearningBarriers} by showing that the selection barrier dominates in block-based environments 
...
both conditions (Enhanced and Original versions of OpenRoberta). The study employed a within subjects design where participants were divided into two groups: Group A experienced the Enhanced version of OpenRoberta first, while Group B started with the Original version. This design allowed us to assess not only the tool's effectiveness but also potential order effects arising from learning transfer between the two conditions.
...
The NASA-TLX results (mean: 1.88, with temporal demand at 1.2 and frustration at 1.7) further support this finding, showing that effective guidance does not require complex interactions. The combination of high SUS scores and low NASA-TLX scores demonstrates that the Reuse Assistant successfully reduces barriers without adding mental burden. The key to this success is directly showing users duplicate code patterns through visual highlighting and providing one-click refactoring, rather than adding complexity.
...
\subsubsection{Minimize Interaction Complexity}
The exceptionally low NASA-TLX scores (temporal demand: 1.2, frustration: 1.7) demonstrate that effective guidance need to not be complex. The Reuse Assistant succeeded through:
...
While the within subjects design allowed within-subjects comparison, the significant order effect (p=.008) indicates that the sequence of 
...
The within subjects design, despite this limitation, provided valuable insights about how prior experience shapes users' ability to benefit 
...
\textit{Mitigation:} We included chemistry domain experts as participants rather than generic users, and the task was based on actual laboratory procedures. However, long-term field studies observing the Reuse Assistant in authentic work contexts are necessary to validate its practical impact.
...
all participants successfully created reusable blocks when help from the Reuse Assistant was available. The feature received high usability ratings (SUS mean: 84.1) and low 
...
Future research should test the Reuse Assistant in real laboratory settings over extended periods to determine whether users learn to  recognize reuse opportunities independently. Studies with more participants from diverse backgrounds would help identify which user 
groups benefit most from automated guidance. The feature should also be evaluated with more complex programming tasks and tested in other end-user programming environments beyond OpenRoberta.
