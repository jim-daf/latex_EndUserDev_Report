...
This paper addresses these three questions:
\begin{itemize}
    \item RQ1: How does OpenRoberta with the Reuse Assistant compare to the standard OpenRoberta in terms of task performance?
    \item RQ2: How well does the Reuse Assistant perform in terms of usability?
    \item RQ3: To what effect does the Reuse Assistant promote the creation and usage of reusable custom blocks?
    \begin{itemize}
        \item Which factors influenced the end user's decision on whether or not to make use of the Reuse Assistant?
    \end{itemize}
\end{itemize}
...
%This results in a learning barrier [cite KO learning barrier],  
As lab work in chemistry involves many repetitive tasks, these challenges can easily become an obstacle for the chemists, which may turn them away from using cobots, as the inconvenience outweighs the benefits.
...
\begin{enumerate}
    ...
    \item \textbf{Survey Feedback:} Collected via a post-experiment survey designed to capture demographic data and subjective perceptions of the effectiveness and usability of the block creation guidance feature.
\end{enumerate}
...
\subsubsection{Attention Investment Model} lol
...
efficacy of automated guidance mechanisms in promoting software reuse among chemistry students and 
educators engaged in laboratory experimentation.

\subsubsection{Overall User Preference}
...
Noteably, 9 out of 10 participants commented on how the enhanced version let them perform their task 
faster. As described in section \ref{sec:background_relatedWork}, this is also one of the main benefits of reuse in the field of 
...
of automating the custom block creation process. In the standard environment, creating a reusable component requires
a manual, multi-step process of defining a custom block. But without creating a custom reusable component, 
the end-users had to manually import the repetitive block sequences to the workspace. 
The enhanced version streamlined this by automating the structural setup of the custom block once a 
duplicate sequence of blocks was detected. 
%This confirms that removing the "friction" of manual block assembly is crucial for encouraging reusability among the end-users.

\subsubsection{Importance of Visual Highlighting}
The user preference for the \textit{Animated Color Highlight} (50\% preference) and the high satisfaction 
rates (90\% satisfied/very satisfied) for the visual highlight underscore the 
importance of clear visual signals to the end users. 
In a dense visual environment like OpenRoberta, static cues are easily overlooked. The dynamic nature 
of the animation highlight acted as a "Just-in-Time" trigger, interrupting the user's tunnel vision exactly when 
the redundancy occurred. This supports the use of proactive, visually distinct interruptions in 
educational block based environments to successfully guide and take the attention of the end users.
...
systems that detect inefficient patterns (such as code duplication) and proactively intervene with suggestions.
...
block sequences and showing how to improve that, the tool created a "learning moment" exactly when the user needed it. 
This suggests that automation reuse assistants can have two benefits: they help domain experts work faster, but they also teach 
beginners difficult concepts like how to organize blocks of code by simply showing it to them in practice.

\subsection{Threats to Validity}
\subsubsection{External Validity - Convenience Sampling} 
The participants to the study were either aquantiances of one of the authors of the study, or were recruited through these aquantiances.
As the study is focused on the domain of chemistry rather than general use, this did leade to a smaller sample pool of possible participants. As such, the results of this study cannot be generalized.
\subsubsection{Internal Validity - Learning Effect}
As all participants complete the same task twice, it is inevitable that there will be a carryover effect in regards to learning. This can affect some of the results, such as task completion time. Two things were done to mitigate this. Firstly, experiment sequence was mixed between subjects. If the first participant first completed the task using the standard OpenRoberta, then the enhanced version, then the second participant would do it in the opposite order. Secondly, the experiments were balanced, meaning half of the participants did the standard -> enhanced sequence, while the other half did enhanced -> standard. In this way, the overall effect on the results is minimized.
\subsubsection{Construct Validity - Ambiguous Metrics}
Certain questions in the survey were too ambigous to provide valid measurements. This included asking participants how 'easy' they found the enhanced version of OpenRoberta, what level of programming knowledge they posess and whether they were satisfied with the highlights. To mitigate this, another questionnaire was created, based on [XX], which the participants were asked to answer less than one week post-experiment.
\subsubsection{Interpretive Validity - Ambiguous Answers}
Certain answers to the open-ended questions were ambigous. The most prominent example is the question about identified differences in the two versions of OpenRoberta. While multiple participants talked about 'it' being faster, they failed to explicitly mention which version of OpenRoberta they were referring to. This was mitigated by asking the participants to clarify which version they were referring to post-experiment. 
