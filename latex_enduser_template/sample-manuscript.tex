...
\begin{itemize}
    ...
    \item \textbf{Pattern Key Generation:} For a sliding window of block sequences (ranging from a minimum to a maximum length), it generates a unique "structural pattern key." This key is a hash or string representation of the block types and their connectivity, ignoring specific parameter values.
    
    \item \textbf{Pattern Matching:} The algorithm aggregates sequences by identical pattern keys. If a pattern key appears more than once (frequency $\ge$ 2), it is flagged as a candidate for reuse.
...
\begin{algorithm}[H]
\caption{Sequence Detection and Pattern Key Generation}
\label{alg:detection}
\begin{algorithmic}[1]
\REQUIRE $Blocks$: List of blocks in workspace
\ENSURE $Candidates$: List of duplicate sequences
\STATE $LinearChain \leftarrow$ \textbf{Linearize}($Blocks$)
\STATE $Patterns \leftarrow$ Map$\langle$String, List$\langle$Sequence$\rangle\rangle$
\FOR{$len \leftarrow \text{MinLen}$ \TO $\text{MaxLen}$}
    \FOR{$i \leftarrow 0$ \TO $\text{length}(LinearChain) - len$}
        \STATE $sequence \leftarrow LinearChain[i : i+len]$
        \STATE $sequenceKey \leftarrow$ ``''
        \FORALL{$block$ \textbf{ in } $sequence$}
            \STATE $sequenceKey \leftarrow sequenceKey + block.type$
        \ENDFOR
        \STATE $Patterns[sequenceKey]$.add($sequence$)
    \ENDFOR
\ENDFOR
\STATE $Candidates \leftarrow$ []
\FORALL{$sequenceKey$ \textbf{ in } $Patterns$}
    \IF{$\text{size}(Patterns[sequenceKey]) \ge 2$}
        \STATE $Candidates$.add($Patterns[sequenceKey]$)
    \ENDIF
\ENDFOR
\RETURN $Candidates$
\end{algorithmic}
