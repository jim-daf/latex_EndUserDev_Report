...
% Re-structured section 3 based on the Design Science Methodology structure
...
The system enables the execution of block-based programs on a simulated cobot through a three-tier architecture, as illustrated in Figure 1. The workflow consists of the following stages:
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{ArchitectureH2.png}
  \caption{System architecture showing the execution flow from the Client (Open Roberta UI) through the Flask Backend to the Mujoco Simulator.}
  \Description{A diagram showing three subsystems: Client Side, Server, and Simulator. Data flows from the User dragging blocks, generating headers, sending HTTP POST to the Flask API, translating logic to Python methods, and finally executing commands in Mujoco.}
...
The core of the reuse assistance, located on the Client Side, is the sequence detection algorithm encapsulated in the \texttt{highlightOnlyFunctionCandidates} function. The algorithm operates in several steps:
\begin{itemize}
    ...
    \item \textbf{Parameter Extraction:} Once a duplicate group is identified, the \texttt{extractLiteralParameters} function compares the instances to identify varying literals. These variations are mapped to future function parameters, ensuring the created abstraction is generalized correctly.
...
\begin{algorithm}[H]
\caption{Sequence Detection and Signature Generation}
\label{alg:detection}
\begin{algorithmic}[1]
\REQUIRE $Blocks$: List of blocks in workspace
\ENSURE $Candidates$: List of duplicate sequences
\STATE $LinearChain \leftarrow$ \textbf{Linearize}($Blocks$) \COMMENT{Filter out unconnected literals}
\STATE $Signatures \leftarrow$ Map$<$String, List$<$Sequence$>>$
\FOR{$len \leftarrow MinLen$ \TO $MaxLen$}
    \FOR{$i \leftarrow 0$ \TO length($LinearChain$) - $len$}
        \STATE $seq \leftarrow LinearChain[i : i+len]$
        \STATE $sig \leftarrow$ ""
        \FORALL{$block$ \textbf{in} $seq$}
            \STATE $sig \leftarrow sig + block.type + \text{"|"}$ \COMMENT{Ignore parameter values}
        \ENDFOR
        \STATE $Signatures[sig]$.add($seq$)
    \ENDFOR
\ENDFOR
\STATE $Candidates \leftarrow$ []
\FORALL{$sig$ \textbf{in} $Signatures$}
    \IF{size($Signatures[sig]$) $\ge 2$}
        \STATE $Candidates$.add($Signatures[sig]$)
    \ENDIF
\ENDFOR
\RETURN $Candidates$
\end{algorithmic}
...
Algorithm 1. illustrates the core logic for identifying reusable candidates by abstracting literal values.
...
The user interface is designed to be intuitive and non-disruptive. When the detection algorithm identifies a candidate, the system visually highlights the blocks on the canvas. A non-blocking toast notification appears, prompting the user to confirm the refactoring. If confirmed, the system automatically generates the custom block definition in a dedicated workspace area (handling visibility via \texttt{revealDefinitionWorkspacePane}) and updates the main workspace, replacing the redundant code with concise function calls. This process abstracts the complexity of manual function creation, guiding the user toward modular design practices.

% --- INICIO FIGURA FUSIONADA ---
\begin{figure}[htbp]
  \centering
  
  % Imagen A (Arriba)
  \includegraphics[width=0.85\linewidth]{highlight.png}
  ...
  \vspace{0.2cm} % Pequeño espacio entre las imágenes
  
  % Imagen B (Abajo)
  \includegraphics[width=0.85\linewidth]{custom_component.png}
  ...
  % Un solo caption explicando A y B
  \caption{Guided Reuse Assistant workflow: (Top) The interface detects and highlights duplicate blocks in green. (Bottom) The automated refactoring result, showing the new custom block definition and the simplified main program.}
  \label{fig:ui_interaction}
\end{figure}
% --- FIN FIGURA FUSIONADA ---
