# latex_EndUserDev_Report

Follow this tutorial to setup latex for visual studio code

https://www.youtube.com/watch?v=4lyHIQl4VM8

# Deployment of Reuse Assistant
Frontend -- Open Roberta
https://reuse-assistant-e4dwemeugwfmafem.germanywestcentral-01.azurewebsites.net/ 
Backend -- Flask API
https://mujoco-flask-c5cchvhabcd0fmh9.germanywestcentral-01.azurewebsites.net/ 
Robot Simulator
https://mujoco-flask-c5cchvhabcd0fmh9.germanywestcentral-01.azurewebsites.net/vnc/?autoconnect=true     

# Source Code
Frontend -- Open Roberta
https://github.com/jim-daf/end_user_deploy
Backend -- Flask API+Robot Simulator
https://github.com/jim-daf/end_user_deploy/tree/flask-deploy

For running the simulator locally:
https://github.com/VictoriousAnnro/openroberta-lab_enduser/tree/final_version_EndUserSemesterProject/mj_pick_and_place